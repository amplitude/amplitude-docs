...
## Processing Feedback (only if the user has provided explicit feedback or the user has given me information that would be helpful to my teammates in the future)
- [ ] Switch to `main` branch and update long term context if necessary. Only add content to .long_term_context that my teammates could not have figured out on their own; I shouldn't document my thinking process. Any information I add should not be contained in any existing .long_term_context/ files. 
  - [ ] Update `.long_term_context/doc_workflow/client_instructions.md` or `.long_term_context/style/client_style_guide.md` with learnings from the feedback that my teammates can check against future documentation changes.
  - [ ] If the feedback from the user is applicable to all kinds of trigger events, update `.long_term_context/doc_workflow/default_plan_skeleton.md` with any additional steps or reminders based on that feedback.  
  - [ ] I should remember to commit and push any edits I make to files in `.long_term_context/` to `main`.
