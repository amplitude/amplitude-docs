...
{{partial:admonition type="note" heading=""}}
Values for the `Total Spent` and `# Purchases` metrics are aggregated based on events in the user's stream that contain the `$revenue` property. 
{{/partial:admonition}}
...
{{partial:admonition type="note" heading=""}}
If the difference between `server_received_time` and `client_upload_time` is less than 60 seconds, the `event_time` will not be adjusted and will equal the `client_event_time`. This is done automatically for newer projects, i.e, those with an ID of 243704 or higher. If you want this 60-second cutoff time to apply to an older project, reach out to Amplitude Support.
{{/partial:admonition}}
