...
When you click on a pin's target element, the tour will _always_ advance to the next step. This is different from the "Advanced trigger" setting (described below), which allows you to specify an additional element that can also advance the tour when clicked. The target element's click behavior is built-in and cannot be disabled, while the advance trigger is an optional setting.
{{/partial:admonition}}
