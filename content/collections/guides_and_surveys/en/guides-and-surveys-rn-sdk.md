...
The `serverZone` used to initialize `@amplitude/analytics-react-native` will automatically be used, so there is no need to pass the `serverZone` in `options` argument to `init`
{{/partial:admonition}}
