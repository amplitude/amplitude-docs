...
- **Identify events count toward limits:** When you enable the **Send Users** option in your event streaming destination, any changes to the selected user properties in Amplitude trigger an `Identify` event. These `Identify` events are sent to your event streaming destinations and count toward your event streaming volume limit. Monitor your `Identify` event volume to avoid exceeding your contracted limits.
