...
* Like saved charts, if there is a change in the name of Custom Event or Derived Property, existing syncs that use these will not work as expected because you need to update the sync configs to use the new names. If the underlying definition of the Custom Event or Derived Property change, you do not need to update the sync. Please make sure to check if these transformations are currently used in any syncs before updating their names.
