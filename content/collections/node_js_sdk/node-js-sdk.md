...
{{partial:admonition type="note" title=""}}
This SDK uses the [HTTP V2](/apis/http-v2-api) API and follows the same constraints for events. Make sure that all events logged in the SDK have the `event_type` field and at least one of `deviceId` (included by default) or `userId`, and follow the HTTP API's constraints on each of those fields.
...
{{partial:admonition type="note" title=""}}
If the Identify call is sent after the event, the results of operations are visible immediately in the dashboard userâs profile area, but it don't appear in chart result until another event is sent after the Identify call. The identify call only affects events going forward. For more information, see [Overview of user properties and event properties in Amplitude](/data/user-properties-and-events).
{{/partial:admonition}}
...
The `flush`Â method triggers the client to send buffered events.
...
The `add`Â method adds a plugin to Amplitude client instance. Plugins can help processing and sending events.
...
The `remove`Â method removes the given plugin name from the client instance if it exists.
