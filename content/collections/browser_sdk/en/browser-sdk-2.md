...
| `transport`                | `TransportType \| TransportOptions`. Sets request API to use. Can be a string (`'fetch'`, `'xhr'`, or `'beacon'`) or an object with optional `type` and optional `headers` properties. See [Custom HTTP request headers](#custom-http-request-headers).                                      | `fetch`                                 |
