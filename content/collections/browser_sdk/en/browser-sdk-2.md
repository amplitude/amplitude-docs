...
| `transport`                | `TransportType \| TransportConfig`. Sets the request API to use. Pass a string (`'fetch'`, `'xhr'`, or `'beacon'`), or an object with `type` and `headers` properties to set custom HTTP headers. See [Custom HTTP request headers](#custom-http-request-headers).            | `'fetch'`                               |
