...
| `config.autocapture.elementInteractions` | Optional. Type: `boolean`. Enables/disables element interaction tracking. If `true`, Amplitude tracks clicks and form field interactions. Default value is `false`. Go to [Track element interactions](#track-element-interactions) for more information and configuration options. |
