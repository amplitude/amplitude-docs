...
#### Arrays in user properties[Â¶](#arrays-in-user-properties "Link to this section")
...
##### `prepend` and `append`[Â¶](#prepend-and-append "Link to this section")
...
### User groups[Â¶](#user-groups "Link to this section")
...
### Group identify[Â¶](#group-identify "Link to this section")
...
TheÂ `groupIdentify`Â method accepts a group type string parameter and group name object parameter, and an Identify object that's applied to the group.
...
### Track revenue[Â¶](#track-revenue "Link to this section")
...
## Opt users out of tracking[Â¶](#opt-users-out-of-tracking "Link to this section")
...
## Disable tracking specific fields[Â¶](#disable-tracking-specific-fields "Link to this section")
...
## Set custom user ID[Â¶](#set-custom-user-id "Link to this section")
...
## Logged out and anonymous users[Â¶](#logged-out-and-anonymous-users "Link to this section")
...
## Session tracking[Â¶](#session-tracking "Link to this section")
...
### Get the session ID[Â¶](#get-the-session-id "Link to this section")
...
## Configure HTTP headers[Â¶](#configure-http-headers "Link to this section")
...
## Log events to multiple projects[Â¶](#log-events-to-multiple-projects "Link to this section")
...
## Web attribution[Â¶](#web-attribution "Link to this section")
...
### Track UTM parameters[Â¶](#track-utm-parameters "Link to this section")
...
#### Enable via SDK[Â¶](#enable-via-sdk "Link to this section")
...
### Track referrers[Â¶](#track-referrers "Link to this section")
...
#### Enable via SDK[Â¶](#enable-via-sdk_1 "Link to this section")
...
### First-touch attribution[Â¶](#first-touch-attribution "Link to this section")
...
### Last-touch attribution[Â¶](#last-touch-attribution "Link to this section")
...
### Multi-touch attribution[Â¶](#multi-touch-attribution "Link to this section")
...
### Log captured attribution values via Amplitude[Â¶](#log-captured-attribution-values-via-amplitude "Link to this section")
...
## Google Tag Manager[Â¶](#google-tag-manager "Link to this section")
...
## Troubleshooting and Debugging[Â¶](#troubleshooting-and-debugging "Link to this section")

#### How to debug[Â¶](#how-to-debug "Link to this section")
...
##### Console[Â¶](#console "Link to this section")
...
##### Network Request[Â¶](#network-request "Link to this section")

Use the **Inspect > Network** tab to view all network requests made by your page. Search for the Amplitude request.
[![sdk debugging network request](../../../assets/images/sdk-debuggability-network-request.png)](../../../assets/images/sdk-debuggability-network-request.png)

Please check the response code and ensure that the response payload is as expected.

##### Instrumentation Explorer/Chrome Extension[Â¶](#instrumentation-explorerchrome-extension "Link to this section")
...
#### Common Issues[Â¶](#common-issues "Link to this section")
...
##### AD Blocker[Â¶](#ad-blocker "Link to this section")
...
##### Cookies related[Â¶](#cookies-related "Link to this section")
...
##### CORS[Â¶](#cors "Link to this section")
...
##### Events fired but no network requests[Â¶](#events-fired-but-no-network-requests "Link to this section")
...
## Advanced topics[Â¶](#advanced-topics "Link to this section")

### Dynamic configuration[Â¶](#dynamic-configuration "Link to this section")

Beginning with version 8.9.0, you can configure your apps to useÂ [dynamic configuration](../../dynamic-configuration/).
...
### COPPA Control[Â¶](#coppa-control "Link to this section")
...
### Get the device ID[Â¶](#get-the-device-id "Link to this section")
...
### Set configuration options[Â¶](#set-configuration-options "Link to this section")
...
### Cookie management[Â¶](#cookie-management "Link to this section")

#### Cookies created by the SDK[Â¶](#cookies-created-by-the-sdk "Link to this section")

On initialization, the SDK creates a cookie that begins with the prefixÂ `amp_`Â and ends with this first six digits of your API key.
 For example,Â `amplitude.getInstance().init("a2dbce0e18dfe5f8e74493843ff5c053")`Â would create a cookie with the keyÂ `amp_a2dbce`.
...
#### Disable cookies[Â¶](#disable-cookies "Link to this section")

Disable cookies created by the SDK with theÂ `disableCookies`Â option. When you disable cookies, the JavaScript SDK defaults to usingÂ `localStorage`Â to store its data.
...
#### SameSite[Â¶](#samesite "Link to this section")

The JavaScript SDK defaults to setting the SameSite option on its cookies toÂ `None`. This can be overridden with theÂ `sameSiteCookie`Â option.
 Amplitude recommends using `Lax` unless there are instances where you have third party sites thatÂ `POST`Â forms to your site.

#### HttpOnly cookies[Â¶](#httponly-cookies "Link to this section")
...
#### Upgrade legacy cookies[Â¶](#upgrade-legacy-cookies "Link to this section")

Legacy cookies created by the SDK were larger than the newer, more compact cookies.
 For users that have older cookies, the SDK only removes old cookies and starts using the new cookie format if theÂ `cookieForceUpgrade`Â option is set toÂ `true`.
...
### RequireJS[Â¶](#requirejs "Link to this section")
...
### Cross-domain tracking (JavaScript)[Â¶](#cross-domain-tracking-javascript "Link to this section")
...
- Site 1:Â `www.example.com`
- Site 2:Â `www.example.org`
...
Users who start on Site 1 and then navigate to Site 2 must have the device ID generated from Site 1 passed as a parameter to Site 2. Site 2 then needs to initialize the SDK with the device ID.
 The SDK can parse the URL parameter automatically ifÂ `deviceIdFromUrlParam`Â is enabled.

1. From Site 1, grab the device ID fromÂ `amplitude.getInstance().options.deviceId`.
2. Pass the device ID to Site 2 via a URL parameter when the user navigates. (for example:Â `www.example.com?amp_device_id=device_id_from_site_1`)
3. Initialize the Amplitude SDK on Site 2 withÂ `amplitude.init(AMPLITUDE_API_KEY, null, {deviceIdFromUrlParam: true})`.

### Tracking UTM parameters, referrer, and gclid (JavaScript)[Â¶](#tracking-utm-parameters-referrer-and-gclid-javascript "Link to this section")
...
By default, the SDK carries over existing UTM Parameters and Referrer values at the start of a new session.Â For example,
...
### Callbacks for `logEvent`, `identify`, and `redirect`[Â¶](#callbacks-for-logevent-identify-and-redirect "Link to this section")
...
### Error callbacks[Â¶](#error-callbacks "Link to this section")
...
### `init` callbacks[Â¶](#init-callbacks "Link to this section")
...
### Use sendBeacon[Â¶](#use-sendbeacon "Link to this section")
...
#### Use sendBeacon only when exiting page[Â¶](#use-sendbeacon-only-when-exiting-page "Link to this section")
...
### Device ID lifecycle[Â¶](#device-id-lifecycle "Link to this section")
...
#### When does a device ID change[Â¶](#when-does-a-device-id-change "Link to this section")
...
#### Custom device ID[Â¶](#custom-device-id "Link to this section")

You can assign a new device ID usingÂ `setDeviceId()`. When setting a custom device ID, make sure the value is sufficiently unique. Amplitude recommends using a UUID.
...
#### Get device ID[Â¶](#get-device-id "Link to this section")
...
#### Share current device ID to another instance[Â¶](#share-current-device-id-to-another-instance "Link to this section")
...
### Content Security Policy (CSP)[Â¶](#content-security-policy-csp "Link to this section")
