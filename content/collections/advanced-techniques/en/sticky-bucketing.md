...
* You want to **decrease the percentage rollout** of an experiment (rolled out to half your users, for example) where the treatment group is not performing well. But you **don’t** want users to be moved from either the treatment or the control to the group that never saw either variant (AKA 'off'). Enabling sticky bucketing will keep users in their assigned groups even after you change the percentage rollout. Decreasing percentage rollout will not cause users to move between treatment and control. Neither will increasing percentage rollout.
