...
Amplitudeでは、イベントの使用状況は**クエリ**によって測定されます。 Amplitudeは、クエリをチャート、セグメント、コホートの定義にある**イベントの選択**として定義します。複数のイベントで構成される[カスタムイベント](https://amplitude.com/docs/analytics/charts/event-segmentation/event-segmentation-in-line-events)または指標を使用する場合、**各コンポーネントイベント**のクエリは、個別に集計されます。

![Screenshot_2023-04-24_at_17.47.39.png](/docs/output/img/jp/screenshot-2023-04-24-at-17-47-39-png.png)

[クロスプロジェクトビュー](https://amplitude.com/docs/admin/account-management/portfolio)でクエリされたイベントは、イベントが最初に取り込まれたプロジェクトのクエリ総数に対してカウントされます。 [ユーザールックアップ機能](https://amplitude.com/docs/analytics/user-data-lookup)を使用する場合、またはイベントが[パスファインダー分析](https://amplitude.com/docs/analytics/charts/legacy-charts/legacy-charts-pathfinder)のパスに表示される場合に、クエリはカウント**されません**。
