...
計算プロパティをパーソナライゼーションツールと同期させるには、[プロパティ、推奨、およびサードパーティデスティネーションへのコホートの同期](https://amplitude.com/docs/cdp/audiences/third-party-syncs)に関する記事に従ってください。

計算プロパティは、AmplitudeのAnalytics製品のフィルターとして、コホートとチャートの両方で使用することができます。コホートでフィルターパラメーターとして計算プロパティを選択し、そのコホートを保存するだけです。そこから、Amplitudeチャートでそのコホートをセグメンテーションパラメーターとして選択することができます。それを使用して、異なるコホート間の[ファネルコンバージョン率](https://amplitude.com/docs/analytics/charts/funnel-analysis/funnel-analysis-get-the-most)または[リテンション率](/docs/analytics/charts/retention-analysis/retention-analysis-build)を比較できます：たとえば、ユーザーと異なる平均注文値、最小購入価格などを比較できます。
