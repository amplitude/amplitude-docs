...
新しい実験を作成するには、最初に[デプロイメントを作成し、SDKをインストールします](https://amplitude.com/docs/experiment/workflow/configure)。次に、以下のステップに従ってください。
...
この時点で、Amplitude Experimentが実験の[プラン]タブに移動します。実験目標パネルのバケットユニットと同様に、実験を作成する際に提供した情報が詳細パネルに記入されています。これらをダブルチェックして、正しいことを確認してから、次のフェーズである[実験目標の定義](https://amplitude.com/docs/experiment/workflow/define-goals)に進みます。
