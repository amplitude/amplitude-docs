...
| `setAdvertisingIdForDeviceId()` | `boolean`. Use the Advertising ID on Android if available from Google Play Services. For example, `Amplitude.getInstance().setAdvertisingIdForDeviceId();`. Please check [here](/docs/sdks/analytics/android/android-sdk/#advertiser-id) for the required module and permission.| `false`|
| `setAppSetIdForDeviceId()` | `boolean`. Use the App Set ID (fall back to this `useAdvertisingIdForDeviceId` is used) for device ID. For example, `Amplitude.getInstance().setAppSetIdForDeviceId();`. Please check [here](/docs/sdks/analytics/android/android-sdk/#app-set-id) for the required module and permission.  | `false` |
