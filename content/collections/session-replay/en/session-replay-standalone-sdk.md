...
| `performanceConfig.interaction.timeoutMs` | `number` | No | `undefined` | Maximum time in milliseconds for CSS selector generation during interaction tracking. If selector generation exceeds this time, it throws a timeout error. |
| `performanceConfig.interaction.maxNumberOfTries` | `number` | No | `10000` | Maximum number of attempts to optimize and simplify the CSS selector path during interaction tracking. Higher values may produce shorter selectors but take longer to compute. |
| `performanceConfig.interaction.threshold` | `number` | No | `1000` | Maximum number of CSS selector combinations to test for uniqueness during interaction tracking. If more combinations would be generated, the selector falls back to a simpler strategy. |
