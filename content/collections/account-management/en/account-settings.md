...
- Toggle *Console Logs* on or off to enable or disable the capture of JavaScript console logs, warnings, errors, and network request errors during session replays. When enabled, console logs appear in each replay when you click the console icon to expand the view. This setting applies to future session captures only. Console logs help identify technical issues that may impact user experience.
