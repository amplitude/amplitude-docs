...
---
In Amplitude, the User Management API provides a programmatic solution to provisioning and group management through a public API. With it, you can manage your organizations at scale and integrate the provisioning process with other tools, including identity providers.
...
For a technical guide and spec for interfacing with the SCIM API, refer to the [SCIM API guide](/docs/apis/analytics/scim). It's useful for developers testing the SCIM API, developing scripts that call the Amplitude SCIM API, or constructing one-off requests.
{{/partial:admonition}}
...
* **Import Users/Groups**: Accesses the users and groups within your organization inside Amplitude and adds new users or updates existing users within Okta.
...
3. Enter the API Token: this is the same as the SCIM key provided by Amplitude and referred to earlier. Enter the token in the field and click *Save*. You now have access to user provisioning actions in the *Import*, *Assignment*, and *Push Groups* tabs of the application.

  ![blobid1.png](/docs/output/img/account-management/blobid1-png.png)

4. After Okta verifies the connection, select the provisioning actions that Okta can send to Amplitude. Check any features in the *To App* section of the *Provisioning* tab that fit your needs. It's usually best to select all available features, to ensure that Amplitude's user records closely match that of your Okta directory.
...
1. In your *Access and SSO Settings* tab, ensure that SCIM is enabled. Save the configuration if you need to enable it.
...
3. Copy the new key value and retest the configuration. Refer to the [technical guide](/docs/apis/analytics/scim) if you want to construct your own requests outside of a provider's integration.