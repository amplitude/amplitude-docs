...
Amplitude recommends you follow the principle of least privilege: when you create or edit a role, grant the minimum permissions that enable a user with that role to do their job. Adding "just in case" permissions opens unnecessary security risks. Amplitude's RBAC system is flexible, so you can update roles to add permissions later as needed.

## Create a new role

You must be an org administrator to create roles. Navigate to *Org Settings > Role Management*. This page lists existing roles in your organization and includes a description, the type of role, and the user who last modified it.
...
When you save the role, the permissions update applies immediately to all users with that role assignment. Before you update a role, Amplitude recommends that you audit where your organization uses that role to help minimize disruption.
