...
When sampling is enabled, Amplitude **upsamples metrics** to provide you with highly accurate estimates on every chart and in every analysis.

To oversimplify, Amplitude multiplies your events and users by a sampling factor, equivalent to `(100% / sampling rate)`.

For example, if you are sampling at 10%, Amplitude multiplies tracked events by 10 to give you an accurate estimate of your **true event volume**. This helps every end user in Amplitude focus on analytics without worrying about the sampling rate in use.

Each Amplitude chart shows the sampling rate applied to it. This allows for transparent communication of effective sampling rates.
...
You can see the raw events seen for your project for last month and the month, along with the number of events after sampling. This provides you with real-time access to event volume.
...
You must be an [Admin](/docs/admin/account-management/user-roles-permissions) in your Amplitude organization to make any sampling-related changes.
...
The dynamic sampling rate specifies the frequency with which your data is queried. For example, if you have 50 million active users per year and you set a dynamic sampling rate of 10%, your queried data contains 5 million active users per year. Your event costs are significantly lower, yet you still have more than enough data to generate highly accurate analyses.

3. Next, set your **user property inclusion list**, if desired.

![scale_4.png](/docs/output/img/account-management/scale-4-png.png)

This list acts as a safelist to set aside small, key sub-populations from your sampling process. Users included in these populations are exempt from sampling, and always appear in your data. These populations are defined by the user properties and values you select in this step.
...
Although Amplitude prioritizes [identifying and tracking unique users](/docs/data/sources/instrument-track-unique-users), ingestion-side sampling can sometimes lead to inaccuracies when it comes to anonymous users. For example, if an anonymous user triggers an event from a new device, Amplitude assigns that user a new Amplitude ID and samples based on that new ID. Even if it's determined at a later time that this user was actually a previous user using a new device, there is **no way** to retroactively link the paired events to the particular user's previous Amplitude ID. Since **Amplitude bases the sampling of events on the Amplitude ID at the time of event ingestion**, analyses that rely on user behavior on new devices may be inaccurate or skewed.
...
Customers with high volumes (10M DAUs and more) achieve results within 0.62% accuracy levels at a 5% sampling rate. Amplitude further assumes that any particular analysis would only need to consider 10% of the DAUs to achieve these results. Higher coverage generally results in higher accuracy.
...
For example, if you sample at 10% with 10 million users, it's extremely unlikely you ever see more than 0.62% error in any metric. So, if your retention is 16%, you might see a variance of:
