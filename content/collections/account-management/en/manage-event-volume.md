...
When sampling is enabled, Amplitude **upsamples metrics** to provide highly accurate estimates on every chart and in every analysis.

To oversimplify: Amplitude multiplies your events and users by a sampling factor, equivalent to `(100% / sampling rate)`.

For example, if you're sampling at 10%, Amplitude multiplies tracked events by 10 to give you an accurate estimate of your **true event volume**. This helps every end user in Amplitude focus on analytics without worrying about the sampling rate in use.

Each Amplitude chart shows the sampling rate applied to it, allowing for transparent communication of effective sampling rates.
...
You can see the raw events for your project for last month and the current month, along with the number of events after sampling. This provides real-time access to event volume.
...
The dynamic sampling rate specifies the frequency with which Amplitude queries your data. For example, if you have 50 million active users per year and you set a dynamic sampling rate of 10%, your queried data contains 5 million active users per year. Your event costs are significantly lower, yet you still have more than enough data to generate highly accurate analyses.

3. Next, set your **user property inclusion list**, if desired.

![scale_4.png](/docs/output/img/account-management/scale-4-png.png)

This list acts as a safelist to set aside small, key sub-populations from your sampling process. Users in these populations are exempt from sampling and always appear in your data. These populations are defined by the user properties and values you select in this step.

{{partial:admonition type='note'}}
This process doesn't apply retroactively. Additionally, the following properties aren't supported by the user property inclusion list: `![amplitude_logo.png](/docs/output/img/account-management/amplitude-logo-png.png)User
{{/partial:admonition}}
...
Although Amplitude prioritizes [identifying and tracking unique users](/docs/data/sources/instrument-track-unique-users), ingestion-side sampling can sometimes lead to inaccuracies with anonymous users. For example, if an anonymous user triggers an event from a new device, that user receives a new Amplitude ID and Amplitude samples them based on that new ID. Even if it's later determined that this user was actually a previous user using a new device, there's **no way** to retroactively link the paired events to the particular user's previous Amplitude ID. Because **Amplitude samples events based on the Amplitude ID at the time of ingestion**, analyses that rely on user behavior on new devices may be inaccurate or skewed.
...
For example, if you sample at 10% with 10 million users, you're extremely unlikely to ever see more than 0.62% error in any metric. So, if your retention is 16%, you might see a variance of:
