...
You can see the raw events seen for your project for last month and the current month, along with the number of events after sampling. This gives you real-time access to event volume.
...
This list acts as a safelist to set aside small, key sub-populations from your sampling process. Users included in these populations are exempt from sampling and always appear in your data. Define these populations by the user properties and values you select in this step.
...
Although Amplitude prioritizes [identifying and tracking unique users](/docs/data/sources/instrument-track-unique-users), ingestion-side sampling can sometimes lead to inaccuracies when it comes to anonymous users. For example, if an anonymous user triggers an event from a new device, Amplitude assigns that user a new Amplitude ID and samples them based on that new ID. Even if Amplitude determines later that this user was a previous user using a new device, there is **no way** to retroactively link the paired events to the particular user's previous Amplitude ID. Since **sampling of events is based on the Amplitude ID at the time the event was ingested**, analyses that rely on user behavior on new devices may be inaccurate or skewed.
...
For example, if you sample at 10% with 10 million users, it's extremely unlikely you'll ever see more than 0.62% error in any metric. So, if your retention is 16%, you might see a variance of:
