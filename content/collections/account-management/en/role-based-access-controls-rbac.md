...
The Administrator (Admin) role is the only default role that doesn't support updating permissions. If you require administrators to carry different permissions in your organization, create a new role to reflect those permissions.
{{/partial:admonition}}

Amplitude's default roles cover most common use cases, but every organization has unique structures and responsibilities. Custom roles let your organization fine-tune access for:

* Specialized teams, for example Growth Engineering or Data Governance, with finely scoped permissions.
* Hybrid roles created for employees who straddle functions, for example a product manager who also creates official dashboards and metrics.

This flexibility lets your organization follow the security best practice of providing the least amount of access that enables users to complete their work.
...
In Amplitude, you assign roles to users for each project. Project membership determines access to that project, and roles within the project determine what a user can do.

### Groups

Groups let you manage users at scale. They define the projects that a member of the group has access to, and their role within those projects. Groups most often map to teams in your organization. For example, the Business Intelligence team has a defined set of Amplitude projects where they do their work and a set of permissions they need to do that work. As a result, you may have a group called "Business Intelligence," with access to Project A and Project B, with the `Analyst` role.

#### Group permission prioritization

When you add a user to a group, admins can't change their permissions at the individual level for projects where access comes from that group. This ensures permission consistency and simplifies troubleshooting when determining why a user has certain access levels.
...
| [Group Name] | Amplitude assigned this role through membership in the specified group.        |
