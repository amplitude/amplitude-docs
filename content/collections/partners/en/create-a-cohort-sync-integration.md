...
This doc uses a list-based integration in its examples. If you create a property-based cohort sync integration, some steps may be slightly different than you see here.
...
This determines what users see in the catalog page. The name must be globally unique to all other cohort sync integrations.
...
When you configure Mapping Fields in the Integration Portal, users of your integration see a **Mappings** section in their Destination Settings. This section lets users select any Amplitude user property to map to a field in your destination system. This also allows for custom user identifiers.
...
Expand **Error Classifications**, and click **Add New Error** to add more status codes. Amplitude recommends including as many status codes and sub error codes as you need. These codes and messages make debugging much faster for your end users.
...
For any error response code that's "undefined" at the point of configuration (without failure reason or status code), Amplitude shows an "unclassified" error type with the following error message by default:
...
- **Maximum number of items in each API call (batch size)**: The default is 10,000, but you can specify this. Amplitude recommends a 10,000 user per cohort batch.
...
Avoid rate limiting if possible. If you do have rate limits, for example, 90 requests per second, make them explicit in your user documentation. Amplitude sends out 4 requests in parallel with each request having up to 10,000 users.
...
In the Testing tab, the Destination Settings is identical to what users of your integration see. This form controls the parameters you defined in the headers of the payload.
...
- **USED**: All variables used either in the list users endpoint, add users endpoint, and remove users endpoint.
...
You can also test end to end with the **Test Endpoint** button in the Test Integration section, which automatically runs all necessary tests.

## Release internally

To test in your organization, click **Release Internally**. This enables anyone within your organization to use the integration you defined. The integration is instantly available once you do this.

### Submit your integration

After you finalize your testing, click **Submit** to submit your integration to the Amplitude team. The review process takes about one week. When Amplitude approves your integration, Amplitude notifies you by email and your integration tile becomes visible in the Destination section of Amplitude.
