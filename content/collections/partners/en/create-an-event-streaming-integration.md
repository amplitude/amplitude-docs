...
- **Regular User Identify Forwarding (creating and updating a user):** Enable Identify Forwarding feature for your destination. This allows the destination to send users and their properties that Amplitude creates under the Identify API in real time. See the [Identify documentation](/docs/apis/analytics/identify) for more information.
- **User Property Mapping:** The user can select an Amplitude user property to map to a field in your destination. This also allows for custom user identifiers.
...
- **Selective Property Forwarding Type:** All custom properties the user forwards are sent as strings.
- **OAuth Authentication:** Amplitude doesn't support OAuth, so partners need to generate an API key.
...
3. **Event limits:** Make sure that your event size limit is flexible enough for customer use cases.
    ...
    For example, [Intercom](/docs/data/destination-catalog/intercom) has a limit of 120 Event Types and 20 meta (which are event properties) per event types. You must use the event filter to select the specific events they want to forward from Amplitude to Intercom.
    {{/partial:admonition}}

4. **Authentication method:** Amplitude doesn't support OAuth, so partners need to generate their API key.
5. **Make sure the endpoint is flexible to ingest objects in a specific format:** Amplitude generates a list of objects in this specific format. Make sure your endpoint handles this specific payload structure. See the next section for a payload example.
...
This section allows you to define the parameters for use in the URL, headers, and event body.
...
- **Required Checkbox:** If this is checked, the user can't forward events to your destination until they configure this field.
...
- **REST API Headers:** You can customize the API headers type to suit your use case. Some examples include:
    ...
    - **Accept-Charset:** This header is set with the request and tells the server which character sets (for example, UTF-8, ISO-8859-1, Windows-1251) are acceptable by the client.
    - **Cache-Control:** The cache policy the server defines for this response; the client stores a cached response and reuses it until the time the Cache-Control header defines.
...
- **Input:** This object includes the forwarded event and its fields, and Amplitude enriches it with the user's other events and user properties. These fields aren't guaranteed and rely on the way each user has instrumented their events. Amplitude suggests keeping the input fields usage to a minimum and using only the most common ones such as: `user_id`, `event_type`, `event_time`.
...
To test in your organization, click **Release Internally**. This enables anyone within your organization to use the integration you defined. Allow up to 5 minutes for changes to take effect.

## Submit

After you finish testing, click **Submit** to submit your integration to the Amplitude team. The review process takes about one week. When Amplitude approves your integration, Amplitude notifies you by email and your integration tile becomes visible in the Destination section of Amplitude.
