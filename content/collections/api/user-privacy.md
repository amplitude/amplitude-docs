...
| `include_mapped_user_ids`          | When `true`, this parameter returns the valid `user_id` values that correspond to a supplied `amplitude_id`. This only changes the response object. To delete mapped users set with the [User Mapping API](/apis/analytics/user-mapping/), include each `user_id` of the mapped user in the `user_ids` array. |
