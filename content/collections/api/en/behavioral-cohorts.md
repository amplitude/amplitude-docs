...
- The advertised limit for cohort export size is 2 million users. In case cohort size is too big, here are suggestion:
    1. Set up cohort sync to [Amazon Kinesis](https://amplitude.com/docs/data/destination-catalog/amazon-kinesis-cohort)
    2. Set up cohort sync to your [Webhook](https://amplitude.com/docs/data/destination-catalog/cohort-webhooks)
    3. If single user lookup, sync the cohort to Profile API, and use [Profile API](https://amplitude.com/docs/apis/analytics/user-profile#get-cohort-ids) for single user lookup.
