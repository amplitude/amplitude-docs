...
6. Set the sync frequency. You can configure the sync frequency yourself when setting up a source. This frequency determines the interval at which Amplitude pulls data from Databricks.
    ...
    The available sync frequency options vary depending on the data type you're importing (Events, User Properties, Group Properties, Profiles, etc.).
    
    Examples:
    
    - Daily sync: Runs once per day at a specified time.
    - 4-hour sync: Runs every 4 hours continuously.
    
    For daily syncs, you can choose a specific hour in the day. For weekly and monthly syncs (when available), you can choose a specific day and hour.
