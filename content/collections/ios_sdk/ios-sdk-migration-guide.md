...
Existing [maintenance SDK](/sdks/analytics-sdks/ios/ios-sdk) data (events, user/device ID) are moved to the latest SDK by default. It can be disabled by setting `migrateLegacyData` to `false` in the [Configuration](/sdks/analytics-sdks/ios/ios-swift-sdk#configuration).
