---
id: e7b2c4d1-5f6a-4e89-b012-3c4d5e6f7a8b
blueprint: section
title: Pricing
author: 83fbb88a-75e7-45dc-aed3-39c10967893c
template: default
exclude_from_sitemap: false
updated_by: 83fbb88a-75e7-45dc-aed3-39c10967893c
updated_at: 1772556732
---
Amplitude offers several pricing tiers:

* Starter
* Plus
* Growth
* Enterprise

Each tier offers different levels of functionality and areas of expansion. Amplitude also lets you purchase add-on functionality to almost every pricing tier, so you can ensure that you're getting the functionality you need and not paying for functionality you're not using.

Except in rare cases, the Help documentation doesn't attempt to specify the functionality differences between the tiers. If you have specific questions about whether a feature is included in your tier, check out the [Plan Comparison](https://amplitude.com/pricing#:~:text=Contact%20sales-,Analytics,-Understand%20the%20full) table or contact [Sales](https://amplitude.com/sales-contact).


## Tiers build functionality

Every pricing tier provides more functionality than the previous one. This means that the functionality included in the Starter plan is automatically included in the Plus plan. The Plus plan then expands upon the Starter plan. That expansion is both in functionality and in expanded limits. The Enterprise tier provides the most functionality with the largest monthly limits.

For example:

* The Starter plan includes out-of-the-box analytics and templates.
    * The Plus plan includes those analytics and templates and also lets you create unlimited analytics views. You can customize your analytics however you want.
* The Starter plan offers 50k MTUs and up to 10M events.
    * The Plus plan expands this volume to 300k MTUs or 25M events.

The Growth and Enterprise plans expand further upon the Plus plan:

* Growth and Enterprise plans both offer unlimited or custom MTU or event volumes.
* The Growth plan offers advanced behavioral analysis.
* The Enterprise plan offers advanced behavioral analysis as well as Cross-product analysis.

## Enterprise-only functionality

Some functionality is only available for Enterprise customers. Expand the section to understand what parts of Amplitude are Enterprise-only:

{{partial:collapse name="Enterprise-only features"}}
* [Guides & Surveys MAB](/docs/guides-and-surveys/guides/experiments)
* [Advanced branching logic](/docs/guides-and-surveys/guides/conditional-logic)
* [Personalization](/docs/guides-and-surveys/guides/personalize-with-variables)
* [Edit history](/docs/guides-and-surveys/guides/testing)
* [RBAC](/docs/guides-and-surveys/guides/roles-and-permissions)
* [Cross-product session replays](/docs/admin/account-management/portfolio)
* [Flag-level access control](/docs/feature-experiment/project-level-permissions)
* [Mutual exclusion groups](/docs/feature-experiment/advanced-techniques/mutually-exclusive-experiments)
* [Holdout groups](/docs/feature-experiment/advanced-techniques/holdout-groups-exclude-users)
* [CUPED](/docs/feature-experiment/workflow/finalize-statistical-preferences)
* [Multi-armed bandits](/docs/feature-experiment/workflow/multi-armed-bandit-experiments)
* [Flag dependencies](/docs/feature-experiment/under-the-hood/flag-dependencies)
* [Activation recommendations](/docs/data/audiences/recommendations)
* [User Profile API](/docs/apis/analytics/user-profile)
* [Taxonomy API](/docs/apis/analytics/taxonomy)
* [Data access control](/docs/data/data-access-control)
{{/partial:collapse}}

## Add-on functionality

If you're on the Growth or Enterprise plan, you can purchase additional functionality as an add-on.

Expand each section below to understand what is included in each add-on:

{{partial:collapse name="Session Replay"}}
* [Unlimited sessions](/docs/session-replay/session-replay-settings)
* [AI Summary](/docs/analytics/session-replay)
* [Targeted replay capture](/docs/session-replay/targeted-replay-capture)
* [Three-month retention](/docs/analytics/session-replay)
{{/partial:collapse}}

{{partial:collapse name="Feature Experimentation"}}
* [Unlimited Active Experiments](/docs/feature-experiment/overview)
* [Approval workflows](/docs/feature-experiment/managing-flags-and-experiments-with-approvals)
* [Stratified sampling](/docs/feature-experiment/workflow/add-variants)
* [Sticky bucketing](/docs/feature-experiment/advanced-techniques/sticky-bucketing)
* [Experiment templates](/docs/feature-experiment/workflow/experiment-templates)
* [Holdout and mutual exclusion groups](/docs/feature-experiment/advanced-techniques/holdout-groups-exclude-users)
* [Multi-armed bandits](/docs/feature-experiment/workflow/multi-armed-bandit-experiments)
* [Flag dependencies](/docs/feature-experiment/under-the-hood/flag-dependencies)
{{/partial:collapse}}

{{partial:collapse name="Web Experimentation"}}
* [Unlimited Active Experiments](/docs/web-experiment/set-up-a-web-experiment)
* [Approval workflows](/docs/feature-experiment/managing-flags-and-experiments-with-approvals)
* [Stratified sampling](/docs/feature-experiment/workflow/add-variants)
* [Multi-armed bandits](/docs/feature-experiment/workflow/multi-armed-bandit-experiments)
{{/partial:collapse}}

{{partial:collapse name="Guides & Surveys"}}
* [Unlimited Guides & Surveys](/docs/guides-and-surveys/get-started)
* [Advanced, unlimited themes](/docs/guides-and-surveys/themes)
* [Custom templates](/docs/guides-and-surveys/templates)
* [Scheduled nudges](/docs/guides-and-surveys/guides/setup-and-target)
* [A/B testing and MAB](/docs/guides-and-surveys/guides/experiments)
* [Air-traffic control](/docs/guides-and-surveys/throttling)
* [Unlimited Resource Center](/docs/guides-and-surveys/resource-center)
* [Prioritization](/docs/guides-and-surveys/guides/setup-and-target)
* [RBAC](/docs/guides-and-surveys/guides/roles-and-permissions)
{{/partial:collapse}}

{{partial:collapse name="Activation"}}
* [Unlimited automated audience syncs](/docs/data/audiences/third-party-syncs)
* [50 real-time cohort syncs](/docs/data/audiences/third-party-syncs)
* [Unlimited real-time streaming*](/docs/data/audiences/third-party-syncs)
* [Behavioral cohorts API](/docs/apis/analytics/behavioral-cohorts)
{{/partial:collapse}}

{{partial:collapse name="AI Feedback"}}
* [Unlimited feedback records*](/docs/amplitude-ai/ai-feedback)
* [Daily analysis of private sources](/docs/amplitude-ai/ai-feedback)
{{/partial:collapse}}

{{partial:collapse name="Accounts"}}
* [Account or Group-level reporting](/docs/analytics/account-level-reporting)
* [Salesforce](/docs/data/source-catalog/salesforce) and [Segment](/docs/data/source-catalog/segment) integrations
{{/partial:collapse}}

For more information, go to the [Plan Comparison](https://amplitude.com/pricing#:~:text=Contact%20sales-,Analytics,-Understand%20the%20full) table or contact [Sales](https://amplitude.com/sales-contact).