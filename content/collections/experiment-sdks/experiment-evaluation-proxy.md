...
* **Enable local evaluation on unsupported platforms**: Use remote [Evaluation APIs](/docs/experiment-apis/experiment-evaluation-api) and [SDKs](sdks/experiment-sdks) to run local evaluation in your infrastructure.
...
The Evaluation Proxy exposes remote [Evaluation API](/docs/experiment-apis/experiment-evaluation-api) and [SDK](/docs/sdks/experiment-sdks) endpoints to run local evaluation within your cluster. This is useful to enable platforms and languages which aren't supported by local evaluation SDKs. As an added benefit, fetch requests made to the evaluation proxy can target cohorts of users, and have assignment events tracked automatically to Amplitude.
